...
class MediaCommands(commands.Cog):
    ...
    async def image_search(self, interaction : discord.Interaction, topic:str):
        response = requests.get(f"https://pixabay.com/api/?key={settings.pixabay_api}&q={topic}")
    ...
    async def search_video(self,interaction: discord.Interaction, topic: str):
        ...
        response = requests.get(f"https://pixabay.com/api/videos/?key={settings.pixabay_api}&q={topic}")
        if response.status_code != 200:
        ...
        try:
            ...
            custom_directory = "root/modules/temp_videos"
