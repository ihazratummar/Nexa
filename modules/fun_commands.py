...
class Fun(commands.Cog):
    ...
    async def gif(self, ctx:commands.Context, tag: str):
        url = f"https://api.giphy.com/v1/gifs/random?api_key={settings.giphy_api}&rating=g&tag={tag}"
        response = requests.get(url)
    ...
    async def cat(self, ctx: commands.Context, user: discord.Member = None):
        ...
        reponse = requests.get(url, headers={'x-api-key': f"{settings.cat_api_key}"})
    ...
    async def dog(self, ctx: commands.Context, user: discord.Member = None):
        ...
        reponse = requests.get(url, headers={'x-api-key': f"{settings.dog_api_key}"})
    ...
    async def fact(self, ctx: commands.Context, user: discord.Member = None):
        ...
        response = requests.get(url, headers={'X-Api-Key':f'{settings.api_ninja}'})
        if response.status_code != 200:
            await ctx.send("Connection Failed")
